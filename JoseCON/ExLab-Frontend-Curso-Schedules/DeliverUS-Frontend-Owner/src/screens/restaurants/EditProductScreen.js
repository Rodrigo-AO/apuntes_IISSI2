...
export default function EditProductScreen ({ navigation, route }) {
  ...
  const [schedulesOpen, setSchedulesOpen] = useState(false) // Para abrir/cerrar el desplegable
  const [schedules, setSchedules] = useState([]) // Para guardar los horarios del backend

  const [initialProductValues, setInitialProductValues] = useState({ name: null, description: null, price: null, order: null, productCategoryId: null, availability: null, image: null, restaurantScheduleId: null })
  ...
  useEffect(() => {
    async function fetchSchedules () {
      try {
        // Obtenemos los horarios usando el id del restaurante que viene en el objeto producto
        if (product) {
          const fetchedSchedules = await getRestaurantSchedules(product.restaurantId)
          const schedulesReshaped = fetchedSchedules.map((s) => {
            return {
              label: `${s.startTime} - ${s.endTime}`,
              value: s.id
            }
          })
          // Añadimos la opción de "No programado" al principio
          setSchedules([{ label: 'Not scheduled', value: null }, ...schedulesReshaped])
        }
      } catch (error) {
        alert(error)
      }
        ...
        <ScrollView>
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: '60%' }}>
              ...
              <DropDownPicker
                open={schedulesOpen}
                value={values.restaurantScheduleId}
                items={schedules}
                setOpen={setSchedulesOpen}
                onSelectItem={item => {
                  setFieldValue('restaurantScheduleId', item.value)
                }}
                placeholder="Select a schedule"
                containerStyle={{ height: 40, marginBottom: 10, zIndex: 900 }} // zIndex alto para que no se oculte
                style={{ backgroundColor: GlobalStyles.brandBackground }}
                dropDownStyle={{ backgroundColor: '#fafafa' }}
              />
              <ErrorMessage name={'restaurantScheduleId'} render={msg => <TextError>{msg}</TextError> }/>
