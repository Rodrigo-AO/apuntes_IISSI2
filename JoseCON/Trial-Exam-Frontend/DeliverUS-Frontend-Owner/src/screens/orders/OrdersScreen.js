...
export default function OrdersScreen ({ navigation, route }) {
  ...
  const fetchRestaurantAnalytics = async () => {
    try {
      ...
      showMessage({
        message: `There was an error while retrieving analytics details (id ${route.params.id}). ${error}`,
        type: 'error',
  ...
  const handleNextStatus = async (order) => {
    try {
      ...
      showMessage({
        message: `There was an error advancing order ${order.id}. ${error}`,
        type: 'danger',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
      ...
      <View style={styles.analyticsContainer}>
          <View style={styles.analyticsRow}>
            <View style={styles.analyticsCell}>
              ...
              <TextSemiBold textStyle={styles.text}>
                {analytics?.invoicedToday != null ? Number(analytics.invoicedToday).toFixed(2) + '€' : '0.00€'}
              </TextSemiBold>
            </View>
            <View style={styles.analyticsCell}>
              <TextRegular textStyle={styles.text}>
          ...
          <View style={styles.analyticsRow}>
            <View style={styles.analyticsCell}>
                ...
                <TextSemiBold textStyle={styles.text}>
                  {analytics?.numDeliveredTodayOrders ?? 0}
                </TextSemiBold>
              </View>
              <View style={styles.analyticsCell}>
                <TextRegular textStyle={styles.text}>
                ...
                <TextSemiBold textStyle={styles.text}>
                  {analytics?.numYesterdayOrders ?? 0}
                </TextSemiBold>
              </View>
          </View>
        </View>
  ...
  const renderOrder = ({ item }) => {
    ...
    >
      ...
      <TextRegular numberOfLines={1}>Address: {item.address ?? 'No address'}</TextRegular>
