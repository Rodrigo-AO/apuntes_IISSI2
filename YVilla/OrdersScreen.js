...
export default function OrdersScreen ({ navigation, route }) {
  ...
  const [orders, setRestaurantOrder] = useState({})
  ...
  const handleNextStatus = async (order) => {
    try {
      const updatedOrder = await update(order.id, nextStatus(order).status)
      showMessage({
          ...
          <View style={styles.analyticsRow}>
            <View style={styles.analyticsCell}>
              ...
              <TextSemiBold textStyle={styles.text}>
                {analytics.invoicedToday ? analytics.invoicedToday.toFixed(2) + '€' : '0.00€'}
              </TextSemiBold>
            </View>
            <View style={styles.analyticsCell}>
              <TextRegular textStyle={styles.text}>
